...
//    Them San Pham vao cart trong sesion
    ...
    arrrItem = [...new Set(arrItem)];
    for (let id of arrrItem){
        // console.log({ id });
        let userID
        let infoProductOfArr = await PRODUCT_MOD.getId(id , userID)
      
        productHasConfBest[productHasConfBest.length] = infoProductOfArr.data;
...
// console.log({ productHasConfBest });
    ...
    // console.log({cart1 : req.session.cart});
    
    return res.json({ data: req.session.cart ,dataRecomender :productHasConfBest  })
})
/*
=====================Tru san Pham=======================
